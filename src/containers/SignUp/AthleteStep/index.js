...
const AthleteStep = props => {
  ...
  const _validateForm = () => {
    ...
    if (!_.isDate(dobValue)) {
      ...
      errors.dob = 'You should contact to your parent for signup in ATHES.';
  ...
  const handleEligibleButton = () => {
    dispatch(
      deleteUserRequest(user.id, res => {
        dispatch(backScreen(''));
        util.logoutUser();
      }),
    );
  ...
  return (
    <ScreenWrapper
      ...
      <ModalView
        ...
        modalButtonPress={handleEligibleButton}
        // image={Image}
        heading={'Age Eligibility'}
        description={
          'Under the ATHES user policy, children under the age of 13 need to be signed up under a parent account.'
        }
        buttonText={'OK'}
      />
    </ScreenWrapper>
